...
- Fixed a regression in `PolarsDeltaIOManager` (introduced in `0.27.12`) where a `partition_by` metadata key on a `TimeWindowPartitionsDefinition`-partitioned asset failed with `Cast error: Cannot cast string '2025' to value of Date32 type`. The overwrite predicate previously emitted a `DATE '...'` literal for any time-window partition, assuming a `Date` column; this broke non-`Date` partition columns (e.g. a string `year` derived from `%Y`). The predicate now compares with `=`/`OR` using plain string literals, which DataFusion coerces to the column's type, working uniformly across string, `Date`, `Datetime`, and integer partition columns. Closes [#330](https://github.com/dagster-io/community-integrations/issues/330).
